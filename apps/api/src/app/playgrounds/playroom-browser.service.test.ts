...
describe('PlayroomBrowserService', () => {
  ...
  describe('linkPlayground()', () => {
    ...
    test('throws NotFoundException if target does not exist', async () => {
      ...
      await expect(service.linkPlayground('project')).rejects.toThrow(NotFoundException);
      expect(mockAccess).toHaveBeenCalledWith(expect.stringContaining('project'));
    });
  ...
  describe('getCurrentLink()', () => {
    test('returns content of .current_playground', async () => {
      ...
      expect(mockReadFile).toHaveBeenCalledWith(expect.stringContaining('.current_playground'), 'utf8');
    });
